class DashboardApp {
    ...
    setupNavigation() {
        ...
        if (bottomNav) {
            ...
            links.forEach(link => {
                ...
                a.id = `bottom-nav-${link.id}`;
                a.innerHTML = `<i class="fa-solid ${link.icon}"></i> <span>${link.text}</span>`;
                a.onclick = (e) => {
                    e.preventDefault();
                    this.navigate(link.id, link.text);
                };
    ...
    navigate(viewId, title) {
        ...
        const activeBottomNav = document.getElementById(`bottom-nav-${viewId}`);
    ...
    async initUserHome() {
        ...
        if (liquid) {
            ...
            liquid.style.background = `linear-gradient(to top, ${color}, #fff)`;
        }
        ...
        if (meta) {
            meta.innerHTML = `
                <div style="font-size: 2.8rem; font-weight: 800; color: var(--primary-dark);">${myBin.fillLevel}%</div>
                <div style="display:flex; gap:10px; margin-top:5px;">
                     ${myBin.isIoT ? `<span class="iot-badge"><i class="fa-solid fa-microchip"></i> ${this.t('live_iot')}</span>` : ''}
                </div>
        ...
        if (footer) {
            footer.innerHTML = `
                <div class="sensor-readout glass-morph">
                    <i class="fa-solid fa-temperature-half" style="color: #ff7043;"></i>
                    <div>
                        <p>${this.t('temperature')}</p>
                        <h4>${myBin.temp || '--'}°C</h4>
                    </div>
                </div>
                <div class="sensor-readout glass-morph">
                    <i class="fa-solid fa-droplet" style="color: #42a5f5;"></i>
                    <div>
                        <p>${this.t('humidity')}</p>
                        <h4>${myBin.hum || '--'}%</h4>
                    </div>
        ...
        // Update Labels in Template (Home Screen)
        const labels = {
            'hero-title': 'smart_bin_status',
            'eco-points-label': 'points',
            'next-pickup-label': 'pickup',
            'quick-actions-title': 'quick_actions'
        };

        const buttons = {
            'req-early-btn': 'request_early',
            'view-history-btn': 'view_history'
        };

        Object.keys(labels).forEach(id => {
            const el = document.getElementById(id);
            if (el) el.innerText = this.t(labels[id]);
        });
        ...
        Object.keys(buttons).forEach(id => {
            ...
            if (el) {
                ...
                el.innerText = ' ' + this.t(buttons[id]);
    ...
    toggleQuantumMode(checked) {
        ...
        if (checked) {
            ...
            document.getElementById('quantum-stats').style.display = 'none';
        }
        // Refresh view
    ...
    async initAdminBins() {
        ...
        tbody.innerHTML = bins.map(bin => {
            return `
            <tr>
                <td data-label="Bin ID"><strong>${bin.id}</strong></td>
                <td data-label="Sector"><small style="padding:4px 8px; background:var(--bg-color); border-radius:4px; font-weight:bold">${bin.sector}</small></td>
                <td data-label="Location">${bin.location}</td>
                <td data-label="Status Index"><span class="status-badge ${bin.status}">${bin.fillLevel}% - ${bin.status.toUpperCase()}</span></td>
                <td data-label="Last Activity">${bin.lastUpdated}</td>
    ...
    showToast(message, type = 'warning', binId = null) {
        ...
        // On mobile, limit stack size to prevent covering the UI
        if (window.innerWidth <= 768) {
            const activeToasts = container.querySelectorAll('.toast:not(.fade-out)');
            if (activeToasts.length >= 2) {
                activeToasts[0].classList.add('fade-out');
                setTimeout(() => activeToasts[0].remove(), 400);
            }
        }
    ...
    initUserSegregation() {
        const labels = {
            'sorting-title': 'ai_classifier',
            'sorting-desc': 'classifier_desc',
            'label-organic': 'organic',
            'label-recyclable': 'recyclable',
            'label-hazardous': 'hazardous',
            'label-green-bin': 'green_bin',
            'label-blue-bin': 'blue_bin',
            'label-red-bin': 'red_bin'
        };
    ...
    setLanguage(lang) {
        ...
        // 4. Update Splash Screen text (if it were still visible)
        const initText = document.querySelector('.splash-content p');
        if (initText) initText.innerText = this.t('initializing');

        // 5. Update Sidebar Bottom (Logout)
        const logoutBtn = document.querySelector('.logout-link');
        if (logoutBtn) logoutBtn.innerHTML = `<i class="fa-solid fa-right-from-bracket"></i> ${this.t('logout')}`;
    }
